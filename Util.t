...
my $tests = {
	Kai => {
		...
		items => {
			q[<li>Gaoler<ch.apos/>s Keys (Special Item)</li>] =>
				=> ("Gaoler<ch.apos/>s Keys"),
		},
	},
	Magnakai => {
		choices => {
...
my $dict_obj;
my $t = new XML::Twig;
for my $level ( keys $tests ) {
	$dict_obj = eval "new Dictionary::$level";
	$level_obj = $tests->{$level};
	for ( keys $level_obj->{choices} ) {
		my $choice = $t->parse($_)->root;
		is( Util::find_conditions($choice, $dict_obj)->{label},
			$level_obj->{choices}{$_}, "$level: choice conditions" );
	}
	for ( keys $level_obj->{items} ) {
		my $item = $t->parse($_)->root;
		is_deeply( Util::find_items($item, $dict_obj),
			$level_obj->{items}{$_}, "$level: items in text" );
	}
}
